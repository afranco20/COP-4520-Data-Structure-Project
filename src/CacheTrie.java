...
public class CacheTrie extends HashTrie {
  ...
  private void inhabit(Cache cache, GenNode val, int hash, int currLevels) {
    if (cache == null) {
      if (currLevels >= 12) {
  ...
  private void sampleLevels(int hash, int level, GenNode curr, CacheNode stats) {
  ...
  private void recordCacheMiss() {
    ...
    if (head.stats.get().approximateMissCount() > 2048) {
      ...
      int maxVal = head.stats.get().sample.get(0);
  ...
  private boolean completeExpansion(GenNode en, Cache curr) {
  ...
  private SNode lookup(Object key, int hash, int level, GenNode curr, Cache currCache) {
  ...
  private SNode fastLookup(Object key, int hash) {
    ...
    if (curr == null) {
      ...
      if (txn == null) {
        return (((SNode) curr.node).key == key) ? ((SNode) curr.node) : null;
      }
      else {
        return lookup(key, hash, 0, root, currCache);
      }
      ...
      logger.warning("Fast lookup error");
      return null;
    }
  ...
  private void insert(Object key, Object val, int hash) {
    Cache currCache = (cacheHead == null) ? null : cacheHead.get();
  ...
  private boolean insert(
      Object k, Object v, int h, int lev, GenNode curr, GenNode prev, Cache currCache) {
    ...
    if (old == null) {
      ...
      int cacheLevel =
          (currCache == null) ? 0 : 31 - Integer.numberOfTrailingZeros(currCache.root.length() - 1);
    ...
    // If we found an entry with the current hash bits
      ...
      if (txn == null) {
        ...
        // Check if ANode is narrow, create ENode to announce expansion
          ...
          // Check that the parent of the current ANode still contains current and then insert the
          // ENode
      ...
      // Frozen SNode so unable to make changes, returns back up to the ENode in control of this
      // SNode
  ...
  private void fastInsert(Object key, Object value, int hash, Cache curr, Cache prev) {
